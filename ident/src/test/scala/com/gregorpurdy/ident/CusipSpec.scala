...
object CusipSpec extends ZIOSpecDefault {
  ...
  def spec: Spec[Any, Any] = suite("CusipSpec")(
    ...
    test("Parse a bunch of example CUSIPs") {
      // A bunch of test cases obtained from pubic SEC data via a PDF at https://www.sec.gov/divisions/investment/13flists.htm
      val cases = List(
        "25470F104",
        "254709108",
        "254709108",
      ...
      assert(results)(hasSize(equalTo(cases.size)))
    },
